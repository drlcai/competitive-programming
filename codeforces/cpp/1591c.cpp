...
void solve(int tc = 0) {
  ...
  // if (pos.size() % k){
  //   ans += 2 * pos[ pos.size() % k -1 ];
  // }
  
  if (neg.size() != 0 && pos.size() != 0){
    cout << min( ans - abs(neg[neg.size()-1]), ans - pos[pos.size()-1] ) << endl;
  }else if (neg.size() == 0 && pos.size() == 0){
    cout << 0 << endl;
  }else if (neg.size() == 0){
    cout << ans - pos[pos.size() - 1] << endl;
  }else{
    cout << ans - abs(neg[neg.size() - 1]) << endl;
  }
