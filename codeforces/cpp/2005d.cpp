...
{
  if (ga <= 0 || gb <= 0)
    return 0; // invalid fixed gcd(s)
  if (b[1] % gb)
    return 0; // ivanlid gcd of b

  for (int i = 0; i <= n; i++)
    for (int j = 0; j < 3; j++)
      dp[i][j] = 0; // intializing
  ...
  for (int i = 2; i <= n; i++)
  {
    for (int j = 0; j < 3; j++)
    {
      for (int k = max(0, j-1); k <= j; k++)
      {
        ...
        if (j == 1)
          swap(a[i], b[i]); // if swapping

        if (a[i] % ga)
          flag = 0; // checking sufficiency
        if (b[i] % gb)
          flag = 0; // checking sufficiency
...
int main()
{
  ...
  while (t--)
  {
    ...
    for (int i = 1; i <= n; i++)
    { // calculating the gcd-s of prefixes and suffixes of a and b
      pa[i] = gcd(pa[i - 1], a[i]);
      pb[i] = gcd(pb[i - 1], b[i]);
    ...
    for (int i = 2; i <= n; i++)
    {
      if (pa[i] == pa[i - 1] && pb[i] == pb[i - 1])
        continue; // checking sufficiency of the prefix
    ...
    // if (pa[n] + pb[n] == max_gcd)
    //   ans -= n; // n-1 times updated with j=2 and k=0 and one time with no swaps at all (dp[n][0])
    for (int i = 1; i <= n; i++)
    {
      if (gcd(pa[i], sb[i + 1]) + gcd(pb[i], sa[i + 1]) == max_gcd)
        ans++; // checking prefixes
    }
